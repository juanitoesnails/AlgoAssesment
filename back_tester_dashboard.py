...
def calculate_30_d_volatility(trading_data: pd.DataFrame) -> pd.DataFrame:
    ...
    trading_data["Volatility"] = trading_data["Daily_Return"].rolling(
        window=30
    ).std() * np.sqrt(252)
...
def calculate_max_drawdown(trading_data: pd.DataFrame, initial_capital) -> pd.DataFrame:
    ...
    trading_data["daily_returns"] = (
        trading_data["Portfolio_value"].pct_change().fillna(0)
    )
    ...
    trading_data["Max_Drawdown"] = (
        trading_data["cum_returns"] - cum_max
    ) / cum_max.replace(0, np.nan)
...
class TradingSignals:
    ...
    def sma_crossover(self, short_window: int = 50, long_window: int = 200):
        ...
        buy_condition = (sma_short > sma_long) & (
            sma_short.shift(1) <= sma_long.shift(1)
        )

        sell_condition = (sma_short < sma_long) & (
            sma_short.shift(1) >= sma_long.shift(1)
        )
    ...
    def stochastic_oscillator(
        self, window: int = 14, overbought: int = 80, oversold: int = 20
    ):
    ...
    def parabolic_sar(
        self, acceleration_factor: float = 0.02, max_acceleration: float = 0.2
    ):
...
class Backtester:
    ...
    def _buy(self, avg_signal: int, close_price: float):
        ...
        num_shares = self._calculate_trade_amt(
            avg_signal=avg_signal, close_price=close_price
        )
...
if selected_option == "How to Use":
    ...
    period = st.selectbox(
        "Select Time Period to Test:", ["1y", "2y", "5y", "10y", "ytd"]
    )
    ...
    strategies = st.multiselect(
        ...
        ["SMA", "EMA", "RSI", "MACD", "BB", "Stochastic", "CCI", "SAR"],
    )

    # Initial capital and cash reserve

    initial_capital = st.number_input(
        "Initial Capital ($)", min_value=1000, value=10_000, step=100
    )

    cash_reserve = st.number_input(
        "Cash Reserve (%)", min_value=0, max_value=100, value=10
    )
    ...
    def display_strategy_params(strategy):
        ...
        if strategy == "SMA":
            ...
            params["window"] = st.number_input(
                ...
                max_value=30,
                value=14,
                help="Window for the RSI calculation",
            )
